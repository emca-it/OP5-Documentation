...
def setup_get_request(provider, aggregation, mode):
    ...
    # Defaults to 5 minutes if no time is found in the file
    if start_time == 0:
    ...
    period = int(period.total_seconds() / MINUTE_IN_SECONDS)
    ...
    filter = " and ".join([
        ...
        "timeGrain eq duration'PT{}M'".format(period)
    ])
