...
+--------------------------+----------------------------------------+
| Service and host filters | - Is the host or service in scheduled downtime or not? |
...
+--------------------------+----------------------------------------+
| Contact filters          | - Do the contact notifications options says that this type of notification is supposed to be sent? |
...
Note: `command_line` should be one line. The line breaks and continuation are for demonstration purposed only.

``` {.bash}
command_name=host-notify
command_line=$USER3$/notify/notify \
             ...
             -p "$CONTACTPAGER$" \
                    ...
                    "LASTHOSTUP=$LASTHOSTUP$" \
                    "LASTHOSTDOWN=$LASTHOSTDOWN$" \
