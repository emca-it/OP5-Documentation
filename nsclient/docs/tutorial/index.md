...
It eliminates the need for SNMP. It goes far beyond so-called agentless approaches, such as WMI, by allowing execution of scripts and programs locally and returning results.

!!! note
    This program is not designed to be used as a standalone. Instead it is designed to be used in combination with a monitoring solution, such as OP5.
    While NSClient++ was designed to be used with OP5 and Nagios it aims to be open and as such supports many protocols so it can be integrated with various other monitoring solutions.

To facilitate NSClient++ support a series of protocols:

| Protocol | Paradigm | Comments                                                                              |
|----------|----------|---------------------------------------------------------------------------------------|
| NRPE     | Active   | Protocol used by OP5 to speak with agent like NSClient++                              |
| NSCA     | Passive  | Protocol used with OP5 so agent can speak to Nagios (reverse way of communication)    |
...
In this document we will cover NRPE and NSCA, as well as show how to integrate these protocols with OP5.
...
To use NSClient++ you need a monitoring system deployed in your network. Many people use OP5. Strictly speaking, NSClient++ is designed to work with any monitoring tool which can execute external scripts or support any of the standard protocols.

When it comes to supported operating systems, NSClient++ was originally designed to run only on Windows. Since version 0.4.1 is can be used on Linux as well.
Currently, most modules are Windows-only. Thus the benefits on other operating systems are limited.
...
Before rushing to install the binaries, you must spend fifteen minutes crafting the configuration file.

!!! danger
    We do recommend that you do not skip this step, as you will have issues later and get frustrated... What are 15 minutes compared to a great monitoring solution?
    If you still can't afford these 15 minutes, take the config example in this doc.

One important thing to understand when it comes to NSClient++ configuration: while most people use initialization ('ini) files, these are not the only possibility. Other options include the Windows Registry and remote files.
But for the reminder of this tutorial we only consider ini files since it is the normal and simplest way.

### Editing

NSClient++ config file is in the well-known INI Format. To edit these you can use any text editor, such as Notepad. Using advanced editors, such as [Notepad++](https://notepad-plus-plus.org/), you can also use such preferable features as syntax highlighting and indentation.

![elevated](images/notepad-administrator.png)

Also note that from Windows Vista onward, INI files reside in a protected space. This means you will need to launch the editor as an administrator to edit them.
...
!!! note
    This module is used by the check_nrpe on OP5.

!!! danger
    Since the default check_nrpe command is very insecure, as it uses unreliable encryption. Since version 0.4.3, NSClient++ has shipped with this disabled.
    Thus it wont work with the legacy check_nrpe unless you enable the **insecure mode**.

All you need to configure is who is allowed to connect as well as how to handle arguments.
...
!!! note
    This is used by NSCA server on Nagios. This is not as common for OP5.
...
Using a domain account is also useful when monitoring specific applications, such as SharePoint or IIS.
